...
void Server::bind_address_to_socket() {
  auto err_code = bind(server_socket_fd_, (sockaddr *)&address_, sizeof(address_));
...
void Server::handle_connections() {
  ...
  while (true) {
    int client_socket = accept(server_socket_fd_, (sockaddr *)&address_, &address_size);
    check_error(client_socket < 0,
                "Couldn't accept the connected socket.\n");
